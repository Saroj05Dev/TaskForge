...
const RemoveMemberModal = ({ open, onClose, member, teamId }) => {
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      onClose();
    } catch (error) { toast.error(error || "Failed to remove member"); }
        ...
        <div>
          ...
          {member && (
            <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed">
              <span className="font-medium text-gray-700 dark:text-gray-300">{member.fullName}</span> will lose access to all tasks shared with this team.
            </p>
          )}
        </div>
      </div>
