...
const inputCls = "w-full px-3.5 py-2.5 border border-gray-200 rounded-xl bg-gray-50 focus:bg-white focus:ring-2 focus:ring-purple-100 focus:border-purple-400 text-sm text-gray-800 placeholder-gray-400 outline-none transition-all duration-200 cursor-text disabled:opacity-50";
...
const InviteMemberModal = ({ open, onClose, teamId }) => {
  ...
  const Icon = <div className="p-2 bg-purple-50 rounded-xl"><UserPlus size={15} className="text-purple-600" /></div>;
      ...
      <form onSubmit={handleSubmit}>
        <div className="px-5 py-5 space-y-4">
          <div>
            ...
            <p className="text-[11px] text-gray-400 mt-1.5">The user must already have a TaskForge account.</p>
          </div>

          <div className="flex items-start gap-2.5 p-3.5 bg-blue-50 border border-blue-100 rounded-xl">
            <div className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-1.5 shrink-0" />
            <p className="text-xs text-blue-700 leading-relaxed">Invited members can view and manage tasks shared with this team.</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/50">
          ...
          <button type="submit" disabled={loading || !email.trim()} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-purple-600 rounded-xl hover:bg-purple-700 transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed shadow-sm">
