      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">View and manage task information</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 self-start sm:self-auto">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white text-sm font-medium rounded-xl hover:bg-purple-700 transition-all duration-150 shadow-sm cursor-pointer"
            >
              <Sparkles size={15} />
              Smart Assign
            </button>
            <button
              onClick={onRefresh}
              className="group flex items-center gap-2 px-4 py-2 border border-gray-200 bg-white text-gray-600 text-sm font-medium rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all duration-150 cursor-pointer"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <div className="px-5 py-5">
                <h2 className="text-xl font-bold text-gray-900 mb-3 leading-snug">{task.title}</h2>

                <div className="flex flex-wrap gap-2 mb-4">
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${PRIORITY[task.priority] ?? "bg-gray-100 text-gray-600 border-gray-200"}`}>
                    {task.priority} Priority
                  </span>
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${STATUS[task.status] ?? "bg-gray-100 text-gray-600"}`}>
                    {task.status}
                  </span>
                </div>
            ...
            {/* Comments */}
            <Card>
              <SectionHeader icon={MessageSquare} iconBg="bg-blue-50 text-blue-600" title="Comments" count={comments.length} />
              <div className="px-5 py-4 space-y-4">
                ...
                <div className="space-y-2">
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-3.5 py-3 border border-gray-200 rounded-xl bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 resize-none text-sm text-gray-700 placeholder-gray-400 outline-none transition-all duration-200 cursor-text"
                  />
                  <button
                ...
                {commentsLoading ? (
                  ...
                  <div className="space-y-3">
                    {comments.map((comment) => (
                      <div key={comment._id} className="group flex items-start gap-3 p-3.5 rounded-xl border border-gray-100 hover:border-gray-200 hover:bg-gray-50 transition-all duration-150">
                        {/* Avatar */}
                        ...
                        <button
                          onClick={() => commentsHook.handleDeleteComment(comment._id)}
                          className="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all duration-150 opacity-0 group-hover:opacity-100 cursor-pointer shrink-0"
          ...
          <div className="space-y-6">
            ...
            {/* Attachments */}
            <Card>
              <SectionHeader icon={Paperclip} iconBg="bg-amber-50 text-amber-600" title="Attachments" count={attachments.length} />
              <div className="px-5 py-4 space-y-3">
                ...
                <div>
                  ...
                  <label
                    htmlFor="file-upload"
                    className="flex items-center justify-center gap-2 w-full px-4 py-3 border-2 border-dashed border-gray-200 rounded-xl hover:border-blue-400 hover:bg-blue-50 transition-all duration-200 cursor-pointer"
                  >
                  ...
                  {attachmentsHook.selectedFile && (
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <button
                        ...
                        disabled={uploadingAttachment}
                        className="p-2 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-xl transition-all duration-150 disabled:opacity-40 cursor-pointer"
                      >
                        <X size={15} />
                      </button>
                    </div>
                ...
                {attachmentsLoading ? (
                  ...
                  <div className="space-y-2">
                    {attachments.map((att) => (
                      <div key={att._id} className="group flex items-center gap-2 p-2.5 border border-gray-100 rounded-xl hover:border-gray-200 hover:bg-gray-50 transition-all duration-150">
                        <Paperclip size={13} className="text-gray-400 shrink-0" />
                        <a
                          href={att.fileUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex-1 text-xs text-blue-600 hover:text-blue-700 hover:underline truncate cursor-pointer"
                        >
                        ...
                        <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button
                            onClick={() => attachmentsHook.handleDeleteAttachment(att.publicId)}
